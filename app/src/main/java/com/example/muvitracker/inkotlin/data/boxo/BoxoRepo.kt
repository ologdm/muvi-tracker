package com.example.muvitracker.inkotlin.data.boxo
...
private constructor(
    ...
    fun getMovieList(callES: EmptyStatesCallbackList<MovieModel>) {
        ...
        callES.onSuccess(getCacheList().map { it.toDomain() }) // 1 carica da locale + trasformo in MovieModel
        ...
        BoxoNetworkDS.callBoxoServer(object : RetrofitCallbackList<BoxoDto> {
            ...
            override fun onSuccess(serverList: List<BoxoDto>) {
                callES.onSuccess(serverList.map { it.toDomain() }) // ES  2 carica da server + trasformo in MovieModel
